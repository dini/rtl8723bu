...
struct cfg80211_bss *rtw_cfg80211_inform_bss(_adapter *padapter, struct wlan_network *pnetwork)
{
	...
	buf = kmalloc(MAX_BSSINFO_LEN, GFP_ATOMIC);
	if (!buf)
		return bss;
...
void rtw_cfg80211_ibss_indicate_connect(_adapter *padapter)
{
	...
	if (!cur_network) {
		DBG_871X("cur_network does not exist!!!\n");
		return;
	}
...
#else
	cfg80211_ibss_joined(padapter->pnetdev, cur_network->network.MacAddress,
...
//#endif
	...
	if( pwdinfo->driver_interface == DRIVER_CFG80211 )
	{
		if (ssids && ssids->ssid_len > 7 && _rtw_memcmp(ssids->ssid, "DIRECT-", 7) &&
...
//#endif //CONFIG_P2P
	...
	if (check_buddy_fwstate(padapter, _FW_UNDER_LINKING|WIFI_UNDER_WPS) == _TRUE) {
		...
		SPIN_LOCK_BH(pwdev_priv->scan_req_lock, &irqL);
		SPIN_LOCK_BH(buddy_wdev_priv->scan_req_lock, &irqL);
		...
		SPIN_UNLOCK_BH(buddy_wdev_priv->scan_req_lock, &irqL);
		SPIN_UNLOCK_BH(pwdev_priv->scan_req_lock, &irqL);
...
static int rtw_cfg80211_monitor_if_xmit_entry(struct sk_buff *skb, struct net_device *ndev)
{
	...
	if ((frame_ctl & RTW_IEEE80211_FCTL_FTYPE) == RTW_IEEE80211_FTYPE_DATA) {
		 ...
		 */
		u16 tmp = le16_to_cpu(dot11_hdr->frame_control);
		if (tmp & 0x0080)
			qos_len = 2;
		if ((tmp & 0x0300) == 0x0300)
			dot11_hdr_len += 6;
...
#endif
{
	...
	_rtw_memcpy((void *)mac, psta->hwaddr, ETH_ALEN);
	sinfo->filled = 0;
	sinfo->filled |= STATION_INFO_SIGNAL;
		...
		if((wps_ie = rtw_get_wps_ie(buf, len, NULL, &wps_ielen)))
		{
			...
			if ( (puconfig_method = (__be16*)rtw_get_wps_attr_content( wps_ie, wps_ielen, WPS_ATTR_CONF_METHOD , NULL, &attr_contentlen)) != NULL )
			{
				...
				if(wdev->iftype != NL80211_IFTYPE_P2P_GO) //for WIFI-DIRECT LOGO 4.2.2, AUTO GO can't set PUSH_BUTTON flags
					*puconfig_method |= cpu_to_be16(WPS_CM_PUSH_BUTTON);
				#endif
			}
